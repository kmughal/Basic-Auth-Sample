      ...
      <div className="main-page__side-menu">
        ...
        {channels && channels.length > 0 && (
          <ul className="projects-container">
            {channels.map((p, index) => (
              <li key={index} className={(p._id === selectedChannel ? "selected-channel" : "")}>
                <Channel channelName={p.channelName} channelId={p._id} />
              </li>
            ))}
          </ul>
        )}
...
function Channel({ channelName, channelId }) {
  return (
    <>
      <a href={`/channels/${channelId}`}>
        <svg width="24" height="10" viewBox="0 0 24 24">
          <path
            fill="currentColor"
            fillRule="evenodd"
            clipRule="evenodd"
